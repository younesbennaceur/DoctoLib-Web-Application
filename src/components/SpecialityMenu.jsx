    ...
    <div className='flex flex-col items-center py-16 text-gray-800 gap-6' id='speciality'>
        ...
        <div className='flex sm:justify-center gap-7 pt-5 w-full overflow-scroll'>
            {specialityData.map((item, index) => {
                return (
                    <Link onClick={()=>{}} className='flex flex-col items-center cursor-pointer flex-shrink-0 hover:translate-y-[-10px] text-sm transition-all duration-500' to={`/doctors/${item.speciality}`} key={index}>
